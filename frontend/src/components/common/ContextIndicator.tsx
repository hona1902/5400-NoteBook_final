  ...
  return (
    <div className={cn('flex-shrink-0 flex items-center justify-between gap-2 py-2 px-3 border-t bg-muted/30', className)}>
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium text-muted-foreground">{t.common.contextLabel}</span>
          ...
          {sourcesInsights > 0 && (
            <Tooltip>
              ...
              <TooltipContent>
                <p>
                  {sourcesInsights !== 1
                    ? String(t.common.insightsForSourcesPlural).replace('{count}', String(sourcesInsights))
                    : String(t.common.insightsForSources).replace('{count}', String(sourcesInsights))
                  }
                </p>
              </TooltipContent>
            </Tooltip>
          ...
          {sourcesFull > 0 && (
            <Tooltip>
              ...
              <TooltipContent>
                <p>
                  {sourcesFull !== 1
                    ? String(t.common.fullSourcesPlural).replace('{count}', String(sourcesFull))
                    : String(t.common.fullSources).replace('{count}', String(sourcesFull))
                  }
                </p>
              </TooltipContent>
            </Tooltip>
          ...
          <>
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <p>
                  {notesCount !== 1
                    ? String(t.common.fullNotePlural).replace('{count}', String(notesCount))
                    : String(t.common.fullNote).replace('{count}', String(notesCount))
                  }
                </p>
              </TooltipContent>
            </Tooltip>
      ...
      {(tokenCount !== undefined || charCount !== undefined) && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          {tokenCount !== undefined && tokenCount > 0 && (
            <span>{formatNumber(tokenCount)} {t.common.tokensLabel}</span>
          )}
          {tokenCount !== undefined && charCount !== undefined && tokenCount > 0 && charCount > 0 && (
            <span>/</span>
          )}
          {charCount !== undefined && charCount > 0 && (
            <span>{formatNumber(charCount)} {t.common.charsLabel}</span>
          )}
        </div>
      )}
    </div>
